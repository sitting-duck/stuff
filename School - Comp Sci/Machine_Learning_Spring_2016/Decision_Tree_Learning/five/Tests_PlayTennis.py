...
class Tests_PlayTennis:

    @staticmethod
    def run_tests(problem):
        Tests_PlayTennis.run_training_set_tests(problem)
        Tests_PlayTennis.run_problem_tests(problem)
        Tests_PlayTennis.run_decision_tree_tests(problem)
        Tests_PlayTennis.run_curiosity_tests()

    @staticmethod
    def run_problem_tests(problem):
        Tests_PlayTennis.test_calculate_information_gain_for_category(problem)
        Tests_PlayTennis.test_calculate_information_gain_for_category_for_undefined_root(problem)
        Tests_PlayTennis.test_calculate_entropy_for_training_set(problem)
        Tests_PlayTennis.test_calculate_entropy_for_attribute(problem)
        Tests_PlayTennis.test_calculate_entropy_for_category(problem)
        Tests_PlayTennis.test_prec(problem)
        Tests_PlayTennis.test_create_decision_tree(problem)
        Tests_PlayTennis.test_get_training_set_partitions_by_attribute(problem)
        Tests_PlayTennis.test_get_node_string(problem)

    @staticmethod
    def run_decision_tree_tests(problem):
        Tests_PlayTennis.test_get_child_nodes_of(problem)
        Tests_PlayTennis.test_get_level_depth_for_node(problem)
        Tests_PlayTennis.test_get_deepest_level(problem)
        Tests_PlayTennis.test_got_correct_decision_tree(problem)

    @staticmethod
    def run_training_set_tests(problem):

        # takes the text data from the input file and returns a 2D list of the training data
        Tests_PlayTennis.test_get_tokenized_data(problem)

        # returns the row including the training data id and class of the training set
        Tests_PlayTennis.test_get_row(problem)

        #returns the column including the category name of the training set
        Tests_PlayTennis.test_get_column(problem)

        # returns a list of the category names in the order they are listed in the training set
        Tests_PlayTennis.test_get_category_names(problem)

        # returns the set of attribute types for a category
        Tests_PlayTennis.test_get_unique_attributes_for_category(problem)

        # returns the set of attribute types from category column in the training data
        Tests_PlayTennis.test_get_unique_attribute_names_from_column(problem)

        # returns the index of the last column of the training set
        Tests_PlayTennis.test_get_index_of_class_column(problem)

        # returns the set of class types for a category
        Tests_PlayTennis.test_get_set_of_unique_class_types(problem)

        # takes an index and returns the category name for that column
        Tests_PlayTennis.test_get_category_name_from_column(problem)

        # takes an index and returns a dictionary with the count for all the attributes for a category set to zero
        Tests_PlayTennis.test_create_attribute_dictionary_from_column(problem)
        ...
        # as the second item
        Tests_PlayTennis.test_get_class_type_frequency_for_attribute_tuple(problem)

        # returns a dictionary containing all the counts of all class types for all attributes of the given category
        Tests_PlayTennis.test_get_class_type_frequency_dictionary_for_category(problem)

        # returns a dictionary containing the counts for all variables in the given training set
        Tests_PlayTennis.test_get_class_type_frequency_dictionary_for_all_categories(problem)

    @staticmethod
    def run_curiosity_tests():
        Tests_PlayTennis.test_copy_double_nested_dictionaries()
    ...
    def test_calculate_information_gain_for_category_for_undefined_root(problem):
        ...
        for i in range(0, len(categories)):
            ...
            assert Tests_PlayTennis.within_acceptable_error(expecteds[i], actual, error), 'test_calculate_information_gain_for_category_for_undefined_root() is broken. expected %s got %s' % (expecteds[i], actual)
    ...
    def test_calculate_entropy_for_attribute(problem):
        ...
        for i in range(0, len(categories)):
            ...
            assert Tests_PlayTennis.within_acceptable_error(expecteds[i], actual, error), 'calculate_entropy_for_attribute() is broken. expected: %s got: %s' % (expecteds[i], actual)
    ...
    def test_get_child_nodes_of(problem):
        ...
        test_tree = Tests_PlayTennis.create_simple_test_tree()
        ...
        expected_child_nodes = Tests_PlayTennis.create_expected_child_nodes_for_root_for_simple_test_tree()
    ...
    def test_get_level_depth_for_node(problem):
        ...
        two_level_test_tree = Tests_PlayTennis.create_two_level_test_tree()
